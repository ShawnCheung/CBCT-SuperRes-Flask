...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    if netG == 'resnet_9blocks':
        ...
        net = Clinical2MicroGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                      n_blocks=9, upsampling_times=sampling_times)
    elif netG == 'micro_to_clinical_resnet_9blocks':
        net = Micro2ClinicalGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                      n_blocks=9, downsampling_times=sampling_times)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netG)
    return init_net(net, init_type, init_gain, gpu_ids)
...
class Micro2ClinicalGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 padding_type='reflect', downsampling_times=3):
        """Construct a Resnet-based generator
